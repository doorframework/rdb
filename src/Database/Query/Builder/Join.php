...
class Join extends Builder {
	 ...
	 * @return  void
	 */
	public function __construct(Database $db, $table, $type = NULL)
	{
	...
	public function compile()
	{
		...
		$sql .= ' '.$this->db()->quote_table($this->_table);

		if ( ! empty($this->_using))
		{
			// Quote and concat the columns
			$sql .= ' USING ('.implode(', ', array_map(array($this->db(), 'quote_column'), $this->_using)).')';
		}
		else
		{
			$conditions = array();
